...
![image-20230528022538037](http://www.iocaop.com/images/2023-05/image-20230528022538037.png)
...
![image-20230529224724300](http://www.iocaop.com/images/2023-05/image-20230529224724300.png)
...
使用无参构造创建`HashMap`以后，会默认创建一个数组长度为16的数组，加载因子为0.75。当我们使用`put`方法添加数据时，不会讲数据直接放到数组中去，而是创建`Entry`对象，记录键值对，并放到数组中去。会根据`Entry`中的`key`计算出`hashcode`，<span style="background-color:pink;">与值无关</span>。
