...
<h2 id="p79-其他知识点-课程没有详细讲-可以看黑马的教程" tabindex="-1"><a class="header-anchor" href="#p79-其他知识点-课程没有详细讲-可以看黑马的教程" aria-hidden="true">#</a> p79-其他知识点(课程没有详细讲，可以看黑马的教程)</h2>
<p>问题说明：</p>
<ul>
<li>幂等性问题：用户对同一操作发起的一次请求或者多次请求是一直的，不会因为多次重复操作产生副作用。简单的说就是<span style = 'background:pink;'>重复提交</span>。单应用中，只需要把数据操作放到事务即可，发生错误立即回滚，但是在响应客户端时，会网络中断或者异常。相当于消息被重复消费了。</li>
<li>消息重复消费，消费者在给MQ返回ack时网络中断，mq没有收到确认信息，这条消息就会被其他消费者消费，或者网络重连时再发给该消费者，造成消息重复消费。</li>
</ul>
<p>解决思路：</p>
<p>生成全局唯一id，每次消费消息时用id来判断该消息是否消费过。</p>
<p>生产端可能会因为各种原因(如业务高峰期)产生重复的消息，那么消费端就要保证<span style = 'background:pink;'>不重复消费消息</span>，意味着同样的消息永远不会被消费多次，实现幂等性。</p>
<p>主流操作：</p>
<ul>
<li>唯一id+指纹码机制</li>
</ul>
<p>​		由某些业务规则或者时间戳，生成唯一信息码，利用查询语句判断这个id是否存在数据库中，就是一个简单的拼接，查询判断是否重复，但是单个数据库写入性能会有瓶颈(可分库分表，但不推荐)。</p>
<ul>
<li>
<p>redis原子性(推荐)</p>
<p>利用setnx的幂等性。</p>
...
<h2 id="p80-优先级队列-场景说明" tabindex="-1"><a class="header-anchor" href="#p80-优先级队列-场景说明" aria-hidden="true">#</a> p80-优先级队列-场景说明</h2>
<p>使用场景：订单催付，比如在tmall下单后，如果用户在规定时间内没有付款，就发短信或推送。就需要区分大客户和小客户，利润大的就优先推送处理。曾经是用redis存放，然后定时轮询，redis的List只能做简单的消息队列，不能够实现优先级的场景。用rabbitMQ可以进行改造和优化，大客户就给高的优先级，否则是默认优先级，进行推送。</p>
<blockquote>
<p>消费者在消费消息时，按照队列中消息的优先级，重新排序，再优先消费优先级最高的消息。</p>
</blockquote>
<p>优先级是有顺序的，0-255，数字越大越越优先。</p>
<p><img src="http://www.iocaop.com/images/2022-09/202209161125554.png" alt="image-20220916112553368"></p>
<h2 id="p81-优先级队列-代码实现" tabindex="-1"><a class="header-anchor" href="#p81-优先级队列-代码实现" aria-hidden="true">#</a> p81-优先级队列-代码实现</h2>
<p>创建队列时，设置队列优先级的最大值，然后发送消息时给消息设置优先级(需要在队列最大优先级之内)。</p>
<blockquote>
<p>需要注意的是。生成者发送消息时要把所有消息都发送到优先级队列里来，再由消费者消费，不然发一条消费一条，队列中的消息没有排序。</p>
</blockquote>
<p>生成者代码：</p>
<div class="language-java ext-java line-numbers-mode"><pre v-pre class="language-java"><code><span class="token doc-comment comment">/**
 * 优先级队列生产者
 *
 * <span class="token keyword">@author</span> lzc
 * <span class="token keyword">@date</span> 2022/09/16
 */</span>
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Product</span> <span class="token punctuation">{</span>

    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">final</span> <span class="token class-name">String</span> <span class="token constant">PRIORITY_QUEUE_NAME</span> <span class="token operator">=</span> <span class="token string">"priority_queue_name"</span><span class="token punctuation">;</span>

    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token keyword">throws</span> <span class="token class-name">IOException</span><span class="token punctuation">,</span> <span class="token class-name">TimeoutException</span> <span class="token punctuation">{</span>
        <span class="token class-name">Channel</span> channel <span class="token operator">=</span> <span class="token class-name">RabbitMqUtils</span><span class="token punctuation">.</span><span class="token function">getChannel</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
        <span class="token class-name">Map</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token class-name">String</span><span class="token punctuation">,</span> <span class="token class-name">Object</span><span class="token punctuation">></span></span> map <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">HashMap</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token punctuation">></span></span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
        <span class="token comment">// 设置队列的最大优先级，不要太大，会影响性能</span>
        ...
        channel<span class="token punctuation">.</span><span class="token function">queueDeclare</span><span class="token punctuation">(</span><span class="token constant">PRIORITY_QUEUE_NAME</span><span class="token punctuation">,</span> <span class="token boolean">true</span><span class="token punctuation">,</span> <span class="token boolean">false</span><span class="token punctuation">,</span> <span class="token boolean">false</span><span class="token punctuation">,</span> map<span class="token punctuation">)</span><span class="token punctuation">;</span>
        <span class="token keyword">for</span> <span class="token punctuation">(</span><span class="token keyword">int</span> i <span class="token operator">=</span> <span class="token number">0</span><span class="token punctuation">;</span> i <span class="token operator">&lt;</span> <span class="token number">10</span><span class="token punctuation">;</span> i<span class="token operator">++</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
            <span class="token comment">// 发送消息，为消息5设置优先级为5，其他不设置</span>
            <span class="token class-name">BasicProperties</span> properties <span class="token operator">=</span> <span class="token keyword">null</span><span class="token punctuation">;</span>
            <span class="token keyword">if</span> <span class="token punctuation">(</span>i<span class="token operator">==</span><span class="token number">5</span><span class="token punctuation">)</span><span class="token punctuation">{</span>
                properties <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">BasicProperties</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">.</span><span class="token function">builder</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">.</span><span class="token function">priority</span><span class="token punctuation">(</span><span class="token number">1</span><span class="token punctuation">)</span><span class="token punctuation">.</span><span class="token function">build</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
            <span class="token punctuation">}</span>
            channel<span class="token punctuation">.</span><span class="token function">basicPublish</span><span class="token punctuation">(</span><span class="token string">""</span><span class="token punctuation">,</span><span class="token constant">PRIORITY_QUEUE_NAME</span><span class="token punctuation">,</span> properties<span class="token punctuation">,</span> <span class="token punctuation">(</span><span class="token string">"消息"</span><span class="token operator">+</span>i<span class="token punctuation">)</span><span class="token punctuation">.</span><span class="token function">getBytes</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>

        <span class="token punctuation">}</span>
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>运行后到控制台可以看到队列创建，收到消息。</p>
<p><img src="http://www.iocaop.com/images/2022-09/202209161223974.png" alt="image-20220916122319874"></p>
<p>消费者代码：</p>
<div class="language-java ext-java line-numbers-mode"><pre v-pre class="language-java"><code><span class="token doc-comment comment">/**
 * 优先级队列消费者
 *
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Consumer</span> <span class="token punctuation">{</span>
    ...
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token keyword">throws</span> <span class="token class-name">IOException</span><span class="token punctuation">,</span> <span class="token class-name">TimeoutException</span> <span class="token punctuation">{</span>
        <span class="token class-name">Channel</span> channel <span class="token operator">=</span> <span class="token class-name">RabbitMqUtils</span><span class="token punctuation">.</span><span class="token function">getChannel</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
        channel<span class="token punctuation">.</span><span class="token function">basicConsume</span><span class="token punctuation">(</span><span class="token constant">PRIORITY_QUEUE_NAME</span><span class="token punctuation">,</span><span class="token boolean">true</span><span class="token punctuation">,</span><span class="token punctuation">(</span>consumerTag<span class="token punctuation">,</span> message<span class="token punctuation">)</span> <span class="token operator">-></span> <span class="token punctuation">{</span>
        ...
        <span class="token punctuation">}</span><span class="token punctuation">,</span>consumerTag <span class="token operator">-></span> <span class="token punctuation">{</span>
            <span class="token class-name">System</span><span class="token punctuation">.</span>out<span class="token punctuation">.</span><span class="token function">println</span><span class="token punctuation">(</span><span class="token string">"消息取消消费"</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
        <span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>结果：由于消息5的优先级是5，其他消息没有设置优先级，所以消息5优先消费。</p>
<p><img src="http://www.iocaop.com/images/2022-09/202209161228115.png" alt="image-20220916122832033"></p>
<h2 id="p82-惰性队列" tabindex="-1"><a class="header-anchor" href="#p82-惰性队列" aria-hidden="true">#</a> p82-惰性队列</h2>
<p>惰性(lazy)队列：消息保存在磁盘中，消费者消费，需要从磁盘读取到内存中进行消费(速度更慢)。适合消息堆积(如消费者宕机)时使用。消息很多，存在磁盘中则不会对内存造成压力。但是执行性能没有正常队列好，默认是用正常队列。</p>
<p>正常(default)队列：消息保存在内存中</p>
<p>声明队列时添加参数:</p>
<div class="language-java ext-java line-numbers-mode"><pre v-pre class="language-java"><code>map<span class="token punctuation">.</span><span class="token function">put</span><span class="token punctuation">(</span><span class="token string">"x-queue-mode"</span><span class="token punctuation">,</span><span class="token string">"lazy"</span><span class="token punctuation">)</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>内存开销对比：发送100w条消息，每条占1kb的情况下。普通队列占用内存1.2GB，惰性队列占用1.5MB(索引占用)</p>
<h2 id="p83-集群原理" tabindex="-1"><a class="header-anchor" href="#p83-集群原理" aria-hidden="true">#</a> p83-集群原理</h2>
<p><img src="http://www.iocaop.com/images/2022-09/202209161544041.png" alt="image-20220916154404892"></p>
<p>可以像黑色部分，node3和node2连接node1成为集群。在这基础上，node4再连接node3也是可以的。</p>
<h2 id="p84-搭建集群" tabindex="-1"><a class="header-anchor" href="#p84-搭建集群" aria-hidden="true">#</a> p84-搭建集群</h2>
<ul>
<li>
<p>搭建集群之前准备了两台服务器。</p>
<p>分别修改hostname</p>
<div class="language-bash ext-sh line-numbers-mode"><pre v-pre class="language-bash"><code><span class="token function">vim</span> /etc/hostname
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>节点1lzc、节点2oldhu</p>
<p>分别在服务器中加入对方的地址:</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>为什么加？不加后面加入集群命令时会报错。<a href='https://blog.csdn.net/qq_44338476/article/details/123043300'>点击跳转</a></p>
</li>
<li>
<p>停掉两台服务器中的mq</p>
<div class="language-bash ext-sh line-numbers-mode"><pre v-pre class="language-bash"><code><span class="token function">service</span> rabbitmq-server stop
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></li>
<li>
<p>需要确保各节点cookie使用的是同一个值，所以要手动拷贝cookie文件，或者使用远程拷贝</p>
<div class="language-bash ext-sh line-numbers-mode"><pre v-pre class="language-bash"><code><span class="token function">scp</span> /var/lib/rabbitmq/.erlang.cookie root@host:/var/lib/rabbitmq/.erlang.cookie
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p><img src="http://www.iocaop.com/images/2022-09/202209161615724.png" alt="image-20220916161422606"></p>
</li>
<li>
<p>启动 RabbitMQ 服务,顺带启动 Erlang 虚拟机和 RbbitMQ 应用服务(所有节点都需要启动)</p>
<div class="language-bash ext-sh line-numbers-mode"><pre v-pre class="language-bash"><code>rabbitmq-server <span class="token parameter variable">-detached</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></li>
<li>
<p>我们让lzc为主节点，即将oldhu加入到lzc</p>
</li>
<li>
<p>在oldhu服务器运行：</p>
<div class="language-bash ext-sh line-numbers-mode"><pre v-pre class="language-bash"><code><span class="token comment"># rabbitmqctl stop 会将Erlang 虚拟机关闭，rabbitmqctl stop_app 只关闭 RabbitMQ 服务</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>集群状态：</p>
<p><img src="http://www.iocaop.com/images/2022-09/202209161738345.png" alt="image-20220916173818221"></p>
</li>
<li>
<p>为集群创建账户(命令不做注释，之前有使用过)</p>
<div class="language-bash ext-sh line-numbers-mode"><pre v-pre class="language-bash"><code>rabbitmqctl add_user lzc <span class="token number">911823</span>
...
<li>
<p>登录web页面查看(集群搭建完成)</p>
<p><img src="http://www.iocaop.com/images/2022-09/202209161740118.png" alt="image-20220916174042044"></p>
</li>
</ul>
<h2 id="p85-镜像队列-解决什么问题" tabindex="-1"><a class="header-anchor" href="#p85-镜像队列-解决什么问题" aria-hidden="true">#</a> p85-镜像队列-解决什么问题？</h2>
<p>我们搭建完集群后，两台服务器中的队列是独立的(不可复用的)。简单的说，我们在lzc这个节点中创建队列hello，然后lzc节点宕机，连接oldhu节点，是拿不到hello队列的。</p>
<ul>
<li>使用one包中的生产者创建队列，发送消息。结果如下：</li>
</ul>
<p><img src="http://www.iocaop.com/images/2022-09/202209161805343.png" alt="image-20220916180549259"></p>
<ul>
<li>
<p>手动停止lzc节点。（只关掉mq服务，不关虚拟机）</p>
<p><img src="http://www.iocaop.com/images/2022-09/202209161809917.png" alt="image-20220916180905844"></p>
<p>可以从oldhu节点的管理界面看到：</p>
<p><img src="http://www.iocaop.com/images/2022-09/202209161811662.png" alt="image-20220916181152579"></p>
<p>管理界面明确显示了这个队列是属于节点lzc的，所以我们再通过java代码，访问oldhu这个节点去消费该队列的消息时会报错的。</p>
<p><img src="http://www.iocaop.com/images/2022-09/202209161817123.png" alt="image-20220916181731020"></p>
</li>
<li>
<p>再次启动节点lzc(模拟宕机后又恢复)</p>
<p>到管理页面查看，发现消息竟然丢失了(没有被消费，并且是队列持久化的)。</p>
<p>(在集群情况下，即使队列是持久化的，宕机重启后，队列中的消息还是会丢失)</p>
<blockquote>
<p>实验了下：消息也持久化就不会丢失。</p>
</blockquote>
<p><img src="http://www.iocaop.com/images/2022-09/202209161839569.png" alt="image-20220916183938494"></p>
</li>
</ul>
<p>​</p>
<h2 id="p85-镜像队列-使用" tabindex="-1"><a class="header-anchor" href="#p85-镜像队列-使用" aria-hidden="true">#</a> p85-镜像队列-使用</h2>
<p>发消息发给一个节点，消息再备份到其他节点，那么信息就有多份。</p>
<p>在web界面添加策略</p>
<p><img src="http://www.iocaop.com/images/2022-09/202209170007979.png" alt="image-20220917000734825"></p>
<p><img src="http://www.iocaop.com/images/2022-09/202209170007469.png" alt="image-20220917000754393"></p>
<p>配置完以后，修改队列名以mirror开头，查看：</p>
<p><img src="http://www.iocaop.com/images/2022-09/202209170010115.png" alt="image-20220917001024035"></p>
<p><img src="http://www.iocaop.com/images/2022-09/202209170010083.png" alt="image-20220917001044007"></p>
<h2 id="p86-实现高可用负载均衡" tabindex="-1"><a class="header-anchor" href="#p86-实现高可用负载均衡" aria-hidden="true">#</a> p86-实现高可用负载均衡</h2>
<p><img src="http://www.iocaop.com/images/2022-09/202209170020250.png" alt="image-20220917002054127"></p>
<p>搭建步骤：<a href='http://www.iocaop.com/posts/2022-my/%E4%B8%AD%E9%97%B4%E4%BB%B6/haproxy+Keepalive%E9%AB%98%E5%8F%AF%E7%94%A8mq.html'>点击跳转</a></p>
<p>说明：haproxy中配置了多台mq。并且haproxy会有多台备机，当主机宕机以后，可以通过Keepalive将ip漂移到备机，让生产者访问备机的haproxy，再转发到mq。</p>
</div></template>
